    ...
    for(const viewport of viewports){
      ...
      for(const file of pages){
        ...
        if(file==='systems.html'&&viewport.name==='desktop'){
          const catalog=await page.evaluate(()=>({cards:document.querySelectorAll('[data-system-card]').length,visible:document.querySelectorAll('[data-system-card]:not([hidden])').length,logicOptions:document.querySelectorAll('[data-catalog-filter="logic"] option').length,rooms:document.querySelectorAll('[data-system-card] a[href^="system.html?theme="]').length,active:!!document.querySelector('.nav-links a[href="systems.html"].active')}));
          if(catalog.cards!==38||catalog.visible!==38||catalog.logicOptions<8||catalog.rooms<38||!catalog.active)failures.push({theme,file:fixture,selector:'systems-catalog',text:JSON.stringify(catalog),ratio:0,minimum:0});
          await page.locator('[data-catalog-search]').fill('Retro OS');
          const searched=await page.locator('[data-system-card]:not([hidden])').count();
          await page.locator('[data-catalog-reset]').first().click();
          await page.locator('[data-catalog-route="editorial"]').click();
          const routed=await page.evaluate(()=>({visible:document.querySelectorAll('[data-system-card]:not([hidden])').length,url:location.search,pressed:document.querySelector('[data-catalog-route="editorial"]')?.getAttribute('aria-pressed')}));
          if(searched!==1||routed.visible<2||!routed.url.includes('logic=editorial')||routed.pressed!=='true')failures.push({theme,file:fixture,selector:'systems-catalog-interaction',text:JSON.stringify({searched,routed}),ratio:0,minimum:0});
        }
        if(file==='system.html'&&viewport.name==='desktop'){
          const lab=await page.evaluate(()=>({name:document.querySelector('[data-lab-name]')?.textContent,headline:document.querySelector('[data-room-headline]')?.textContent,archetype:document.querySelector('[data-system-room]')?.dataset.roomArchetype,signatures:document.querySelectorAll('[data-room-signatures] li').length,annotations:document.querySelectorAll('[data-room-annotations] span').length,neighbors:document.querySelectorAll('[data-room-neighbors] a').length,route:document.querySelectorAll('[data-room-route] a').length,sectionOrder:[...document.querySelectorAll('[data-system-room]>[data-room-section]')].map(element=>element.dataset.roomSection),swatches:document.querySelectorAll('.lab-swatch').length,patterns:document.querySelectorAll('[data-lab-patterns] li').length,download:!!document.querySelector('[data-download-tokens]'),nav:!!document.querySelector('.nav-links a[href="compare.html"]'),position:document.querySelector('[data-room-position]')?.textContent,prev:document.querySelector('[data-room-prev]')?.getAttribute('href'),next:document.querySelector('[data-room-next]')?.getAttribute('href'),visual:document.querySelector('[data-room-visual]')?.getAttribute('href'),title:document.title,url:location.search}));
          if(!lab.name||!lab.headline||!lab.archetype||lab.signatures!==3||lab.annotations!==3||lab.neighbors!==2||lab.route!==6||lab.sectionOrder.length!==6||new Set(lab.sectionOrder).size!==6||lab.sectionOrder[0]!=='opening'||lab.swatches!==6||lab.patterns<3||!lab.download||!lab.nav||!lab.position?.includes('/ 38')||!lab.prev?.includes('theme=')||!lab.next?.includes('theme=')||!lab.visual?.includes(`theme=${theme}`)||!lab.title.includes(lab.name)||!lab.url.includes(`theme=${theme}`))failures.push({theme,file:fixture,selector:'system-room',text:JSON.stringify(lab),ratio:0,minimum:0});
        ...
        if(file==='compare.html'&&viewport.name==='desktop'){
          ...
          if(compare.selectors!==114||compare.frames.length!==3||compare.frames.some(src=>!src?.includes('embed=1'))||compare.embeddedEditors!==0||!compare.nav||compare.rooms.length!==3||compare.rooms.some(href=>!href?.startsWith('system.html?theme='))||!compare.url.includes('a=retroos'))failures.push({theme,file:fixture,selector:'compare',text:JSON.stringify(compare),ratio:0,minimum:0});
        }
