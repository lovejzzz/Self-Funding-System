...
### 6.2.4 External close 必须分成 provisional API pass 与 evidence-complete report/bank pass

Stripe 的 `BalanceTransaction` 是首版 external cash-movement anchor：它把 gross、fee、net、currency、availability、balance type、reporting category 与 source object 连接起来。PaymentIntent、Refund、Dispute 与 Payout 都必须通过各自对象和 BalanceTransaction 关系接到账本；webhook 只做已认证的触发和证据记录，因为 Stripe 明确说 event 可能乱序、重试、重复，甚至由不同 Event 指向同一 underlying object。OpenRouter generation usage 同样只证明 job-level provider cost accrual，不证明 operator 的外部 payment method 已经被 charge。

首版 close 因此分成两次检查：UTC interval 结束后的 `PROVISIONAL` pass 读取 canonical API objects、Balance Transactions、internal double-entry 和 provider usage，并立即检查 invariant；但它不能释放 surplus。只有 Stripe itemized report 已覆盖完整 interval、standard automatic payout 的 transaction membership 已确定、payout 已与 operator business checking 的 posted line 匹配、provider usage 与 cash document 分开保存，并由不参与准备的 View Only reviewer attestation 后，才进入 `REVIEWED_CLOSED`。Stripe 当前说明 daily report 通常次日可用，所以旧的“完整对账小于 24 小时”目标没有直接证据；首版改用 **36 小时 stale-evidence freeze target**，并用真实账户 latency 替换它。

manual/instant payout 不能假设拥有 automatic payout 的 membership relation，首版因此只使用 standard automatic payout。对首个 $20 experiment，unexplained cash difference tolerance 是零分；但 stop scope 分层：正常时序 exposure 只冻结 affected surplus，任何 unmatched BalanceTransaction、unexpected fee/adjustment、stale report、missing bank payout 或 provider-cost mismatch 冻结新任务、provider/discretionary spend 与 surplus release；unbalanced/duplicate internal booking、unexpected payout destination、negative external balance 或 credential/evidence compromise 停止全部 runtime mutation。退款与客户保护在前两层继续；全停时改走独立认证的人类 emergency path。恢复必须有 source document、append-only reversal/correction、clean rerun 和 independent approval，不能删除旧 entry。
