...
const targetPage=process.env.TEST_PAGE||'index.html';
...
try{
  for(const theme of themes){
    ...
    await page.goto(`${base}/${targetPage}`,{waitUntil:'networkidle'});
  ...
  await contact.screenshot({path:join(output,`${themes.length}-theme-contact-sheet.png`),fullPage:true});
}finally{
  await browser.close();
  server.close();
}

console.log(`Captured ${themes.length} laptop previews and ${join(output,`${themes.length}-theme-contact-sheet.png`)}.`);
